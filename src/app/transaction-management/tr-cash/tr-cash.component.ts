  ...
  async addTransaction(accNumber:string){
   ...
   const transaction = {
    ...
    const account = {
      ...
      acc_balance: await accBalance + parseFloat(this.accForm.get('tr_amount')!.value),
      acc_status: true
      
    }


    if(this.accForm.get('tr_amount')!.value < 0 && await accBalance < -1*(this.accForm.get('tr_amount')!.value)){
      this.errorMessage = "Nincs fedezet! Egyenleg: "+await accBalance;
      this.successMessage = "";
    }
    else if (this.accForm.get('tr_amount')!.value < 0 && await accBalance >= -1*(this.accForm.get('tr_amount')!.value)) {
